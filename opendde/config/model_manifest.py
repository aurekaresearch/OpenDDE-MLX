...
def get_model_manifest(model_name: str, manifest: dict[str, Any] | None = None) -> dict[str, Any]:
...
def get_checkpoint_manifest(
    ...
    raise KeyError(f"Unknown checkpoint {checkpoint_filename!r} for model {model_name!r}")
...
def checkpoint_url(
    ...
    *,
    ...
    if model_name is None:
        ...
        get_checkpoint_manifest(get_model_manifest(model_name, manifest), checkpoint_filename)
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        if args.command == "resolve-model":
            ...
            print(
                f"Verified {filename}: {expected['size_bytes']} bytes, SHA-256 {expected['sha256']}"
            )
