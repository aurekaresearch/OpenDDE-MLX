...
class Hmmalign(BinaryWrapper):
    ...
    def align(self, a3m: str, profile: str, flags: Optional[Mapping[str, str]] = None) -> str:
        """
...
class Hmmbuild(BinaryWrapper):
    ...
    def __init__(
        ...
        self.opts = (["--singlemx"] if singlemx else []) + ([f"--{alphabet}"] if alphabet else [])
    ...
    def query(self, seq: str) -> MsaToolResult:
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            if os.path.getsize(sto_p) > 0:
                ...
                aligned = Hmmalign(binary_path=self.aligner_p).align_sequences_to_profile(prof, a3m)
                final_a3m = "\n".join(
                    f">{d}\n{s}" for s, d in lazy_fasta_parse(f">query\n{seq}\n" + aligned)
                )
            else:
                final_a3m = f">query\n{seq}"
