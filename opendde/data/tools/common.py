...
def align_to_query(seq: str, query: str) -> str:
    ...
    return "".join(s if q != "-" else (s.lower() if s != "-" else "") for s, q in zip(seq, query))
...
def convert_a3m_to_stockholm(a3m: str, max_seqs: Optional[int] = None) -> str:
    ...
    gs_lines = [
        f"#=GS {n} DE {d.partition(' ')[2].strip() or '<EMPTY>'}" for n, d in zip(names, descs)
    ]
    ...
    return "\n".join(
        ["# STOCKHOLM 1.0", ""] + gs_lines + [""] + msa_lines + [f"{'#=GC RF':<{w}} {rf}", "//"]
    )
...
def convert_stockholm_to_a3m(
    ...
    for name, s in seqs.items():
        ...
        final_s = (
            align_to_query(s, query_seq).replace(".", "") if remove_gaps else s.replace(".", "")
        )
        if linewidth:
            chunks.extend(final_s[i : i + linewidth] for i in range(0, len(final_s), linewidth))
        else:
