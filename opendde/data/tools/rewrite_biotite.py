...
def _find_matches(query_arrays, reference_arrays):
    ...
    if query_arrays[0].shape[0] * reference_arrays[0].shape[0] <= FIND_MATCHES_SWITCH_THRES:
...
def _parse_inter_residue_bonds(atom_site, struct_conn):
    ...
    for i in range(2):
        ...
        for col_name in COLUMNS:
            struct_conn_col_name = pdbx_convert._get_struct_conn_col_name(col_name, i + 1)
...
def concatenate(atoms):
    ...
    for element in atoms:
        if element_type is None:
            ...
            if not isinstance(element, element_type):
                raise TypeError(
                    f"Cannot concatenate '{type(element).__name__}' with '{element_type.__name__}'"
                )
    ...
    if has_bonds:
        ...
        concat_atoms.bonds = bond_list_concatenate(
            [
                (element.bonds if element.bonds is not None else BondList(element.array_length()))
                for element in atoms
            ]
        )
