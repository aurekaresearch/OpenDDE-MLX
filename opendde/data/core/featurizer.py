...
class Featurizer(object):
    ...
    def encoder(
        ...
        Returns:
            np.ndarray: A float32 one-hot array of of the input values.
        """
        ...
        indices = np.array([idx_map[item] for item in input_list], dtype=np.int64)
        return np.eye(num_keys, dtype=np.float32)[indices]
    ...
    def ref_atom_name_chars_encoded(atom_names: list[str]) -> np.ndarray:
        ...
        return np.eye(64, dtype=np.float32)[indices.reshape(n, 4)]
    ...
    def get_lig_frame(
        ...
        if kdtree is None:
            ...
            if has_frame:
                ...
                is_zero_norm = np.isclose(np.linalg.norm(vec1, axis=-1), 0) or np.isclose(
    ...
    def get_token_features(self) -> dict[str, np.ndarray]:
        ...
        token_features["token_index"] = np.arange(len(self.cropped_token_array), dtype=np.int64)
        token_features["residue_index"] = centre_atoms.res_id.astype(np.int64)
        token_features["asym_id"] = centre_atoms.asym_id_int.astype(np.int64)
        token_features["entity_id"] = centre_atoms.entity_id_int.astype(np.int64)
        token_features["sym_id"] = centre_atoms.sym_id_int.astype(np.int64)
    ...
    def get_chain_perm_features(self) -> dict[str, np.ndarray]:
        ...
        chain_perm_features = {}
        chain_perm_features["mol_id"] = self.cropped_atom_array.mol_id.astype(np.int64)
        chain_perm_features["mol_atom_index"] = self.cropped_atom_array.mol_atom_index.astype(
            np.int64
        )
        chain_perm_features["entity_mol_id"] = self.cropped_atom_array.entity_mol_id.astype(
            np.int64
        )
    ...
    def get_reference_features(self) -> dict[str, np.ndarray]:
        ...
        ref_features["ref_mask"] = self.cropped_atom_array.ref_mask.astype(np.int64)
        ref_features["ref_element"] = Featurizer.elem_onehot_encoded(
            self.cropped_atom_array.element
        ).astype(np.int64)
        ref_features["ref_charge"] = self.cropped_atom_array.ref_charge.astype(np.int64)
        ...
        ref_features["ref_space_uid"] = self.cropped_atom_array.ref_space_uid.astype(np.int64)
        ...
        ref_features["has_frame"] = np.array(
            token_array_with_frame.get_annotation("has_frame")
        ).astype(np.int64)  # [N_token]
        ref_features["frame_atom_index"] = np.array(
            token_array_with_frame.get_annotation("frame_atom_index")
        ).astype(np.int64)  # [N_token, 3]
        return ref_features
    ...
    def get_bond_features(self) -> dict[str, np.ndarray]:
        ...
        polymer_mask = np.isin(self.cropped_atom_array.mol_type, ["protein", "dna", "rna"])
        std_res_mask = (
            np.isin(self.cropped_atom_array.res_name, list(STD_RESIDUES.keys())) & polymer_mask
        )
        ...
        inter_unstd_bond_mask = (unstd_res_mask[bond_atom_i] & unstd_res_mask[bond_atom_j]) & (
        ...
        bond_features = {"token_bonds": token_adj_matrix.astype(np.float32)}
    ...
    def get_extra_features(self) -> dict[str, np.ndarray]:
        ...
        extra_features["atom_to_tokatom_idx"] = self.cropped_atom_array.tokatom_idx.astype(np.int64)

        extra_features["is_protein"] = self.cropped_atom_array.is_protein.astype(np.int64)
        extra_features["is_ligand"] = self.cropped_atom_array.is_ligand.astype(np.int64)
        extra_features["is_dna"] = self.cropped_atom_array.is_dna.astype(np.int64)
        extra_features["is_rna"] = self.cropped_atom_array.is_rna.astype(np.int64)
    ...
    def get_structural_token_features(
        self, ref_pos: np.ndarray, ref_mask: np.ndarray
    ) -> dict[str, np.ndarray]:
        """
        ...
        structural_token_array = self.inherit_parent_backbone_frames(structural_token_array)

        atom_to_structural_token_idx = np.full(len(self.cropped_atom_array), -1, dtype=np.int64)
        atom_to_structural_tokatom_idx = np.full(len(self.cropped_atom_array), -1, dtype=np.int64)
        structural_rep_atom_mask = np.zeros(len(self.cropped_atom_array), dtype=np.int64)
        prev_parent_residue_idx, next_parent_residue_idx = self.get_polymer_residue_graph()
        ...
        for token_idx, token in enumerate(structural_token_array):
            ...
            structural_prev_parent_residue_idx.append(prev_parent_residue_idx[parent_idx])
            structural_next_parent_residue_idx.append(next_parent_residue_idx[parent_idx])
        ...
        return {
            "structural_token_index": np.arange(len(structural_token_array), dtype=np.int64),
            "residue_token_group_id": np.array(
                structural_token_array.get_annotation("residue_token_group_id"),
                dtype=np.int64,
            ),
            "subtoken_role": subtoken_role_id,
            "subtoken_role_id": subtoken_role_id.copy(),
            "twin_token_idx": np.array(
                structural_token_array.get_annotation("twin_token_idx"),
                dtype=np.int64,
            ),
            "parent_residue_idx": np.array(
                structural_token_array.get_annotation("parent_residue_idx"),
                dtype=np.int64,
            ),
            "atom_to_structural_token_idx": atom_to_structural_token_idx,
            "atom_to_structural_tokatom_idx": atom_to_structural_tokatom_idx,
            "structural_distogram_rep_atom_mask": structural_rep_atom_mask,
            "structural_pae_rep_atom_mask": structural_rep_atom_mask.copy(),
            "structural_has_frame": np.array(
                structural_token_array.get_annotation("has_frame"),
                dtype=np.int64,
            ),
            "structural_frame_atom_index": np.array(
                structural_token_array.get_annotation("frame_atom_index"),
                dtype=np.int64,
            ),
            "prev_parent_residue_idx": np.array(structural_prev_parent_residue_idx, dtype=np.int64),
            "next_parent_residue_idx": np.array(structural_next_parent_residue_idx, dtype=np.int64),
            "structural_is_polymer": np.array(structural_is_polymer, dtype=np.int64),
            "structural_polymer_type": np.array(structural_polymer_type, dtype=np.int64),
            "structural_seq_pos": np.array(structural_seq_pos, dtype=np.int64),
        }

    @staticmethod
    def _is_polymer_backbone_bond(mol_type: str, atom_name_a: str, atom_name_b: str) -> bool:
        atom_pair = {atom_name_a, atom_name_b}
        if mol_type == "protein":
            return atom_pair == {"C", "N"}
        if mol_type in ["dna", "rna"]:
            return "P" in atom_pair and bool(atom_pair.intersection({r"O3'", "O3*", "O3T"}))
        return False
    ...
    def get_polymer_residue_graph(self) -> tuple[np.ndarray, np.ndarray]:
        ...
        centre_atom_indices = self.cropped_token_array.get_annotation("centre_atom_index")
        centre_atoms = self.cropped_atom_array[centre_atom_indices]
        parent_is_std_polymer = np.array(
            [
                atom.mol_type in ["protein", "dna", "rna"] and atom.res_name in STD_RESIDUES
        ...
        if has_bond_graph:
            for bond in self.cropped_atom_array.bonds.as_array():
                ...
                if parent_a < 0 or parent_b < 0 or parent_a == parent_b:
                    continue
                if not (parent_is_std_polymer[parent_a] and parent_is_std_polymer[parent_b]):
                    continue
                ...
                first_parent, second_parent = (
                    (parent_a, parent_b) if parent_a < parent_b else (parent_b, parent_a)
                )
    ...
    def get_mask_features(self) -> dict[str, np.ndarray]:
        ...
        mask_features["pae_rep_atom_mask"] = self.cropped_atom_array.centre_atom_mask.astype(
            np.int64
        )

        mask_features["plddt_m_rep_atom_mask"] = (
            self.cropped_atom_array.plddt_m_rep_atom_mask.astype(np.int64)
        )  # [N_atom]

        mask_features["distogram_rep_atom_mask"] = (
            self.cropped_atom_array.distogram_rep_atom_mask.astype(np.int64)
        )  # [N_atom]

        mask_features["modified_res_mask"] = self.cropped_atom_array.modified_res_mask.astype(
            np.int64
        )
        ...
        for i, j, _ in lig_polymer_bonds:
            bond_mask_mat[i, j] = 1
            bond_mask_mat[j, i] = 1
        mask_features["bond_mask"] = bond_mask_mat.astype(np.int64)  # [N_atom, N_atom]
