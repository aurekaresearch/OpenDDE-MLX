...
from opendde.data.core.substructure_perms import get_substructure_perms
...
def _components_file() -> str:
    return cast(str, data_configs["ccd_components_file"])


def _rdkit_mol_pkl() -> Path:
    return Path(cast(str, data_configs["ccd_components_rdkit_mol_file"]))


def set_ccd_cache_paths(
    components_file: Union[str, Path, None] = None,
    rdkit_mol_pkl: Union[str, Path, None] = None,
) -> None:
    ...
    get_one_letter_code.cache_clear()
...
def biotite_load_ccd_cif() -> pdbx.CIFFile:
    ...
    """
    return pdbx.CIFFile.read(_components_file())
...
def get_component_rdkit_mol(ccd_code: str) -> Union[Chem.Mol, None]:
    ...
    rdkit_mol_pkl = _rdkit_mol_pkl()
    if rdkit_mol_pkl.exists():
        with open(rdkit_mol_pkl, "rb") as f:
            _ccd_rdkit_mols = compat_pickle_load(f)
        return _ccd_rdkit_mols.get(ccd_code, None)
    else:
        from opendde.config.dependency_url import URL

        raise FileNotFoundError(
            f"CCD RDKit mol cache not found: {rdkit_mol_pkl}\n"
            f"Download it from {URL['ccd_components_rdkit_mol_file']} "
            "and place it under $OPENDDE_ROOT_DIR/common/ before running inference."
        )
...
def get_ccd_ref_info(
    ...
    if they are from before the model data cut-off (2021-09-30 unless otherwise stated).
    ...
    Args:
        ...
        ccd_mols (Optional[tuple[tuple[str, Chem.Mol]]]): Self-defined CCD molecules.
        return_atomic_number (bool): Whether to include atomic numbers.
    ...
    Returns:
        Dict:
            ...
            atomic_number: atom atomic numbers, when requested
    """
    if ccd_mols is None:
        ccd_mols_dict = {}
    else:
    ...
    if return_atomic_number:
        results["atomic_number"] = np.array([atom.GetAtomicNum() for atom in mol.GetAtoms()])
...
def add_inter_residue_bonds(
    ...
    if remove_far_inter_chain_pairs:
        if not hasattr(atom_array, "label_asym_id"):
            logger.warning("label_asym_id not found, far inter chain bonds will not be removed")
        for b_idx, (atom_i, atom_j, b_type) in enumerate(inter_bonds._bonds):
