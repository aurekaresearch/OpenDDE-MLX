...
from opendde.data.constants import CRYSTALLIZATION_AIDS
from opendde.data.utils import get_inter_residue_bonds
...
class Filter(object):
    ...
    @staticmethod
    def remove_crystallization_aids(atom_array: AtomArray, entity_poly_type: dict) -> AtomArray:
    ...
    @staticmethod
    def remove_polymer_chains_too_short(atom_array: AtomArray, entity_poly_type: dict) -> AtomArray:
        chain_starts = struc.get_chain_starts(atom_array, add_exclusive_stop=True)
        invalid_chains = []  # list of [start, end)
        for index in range(len(chain_starts) - 1):
            start, end = chain_starts[index], chain_starts[index + 1]
    ...
    def remove_polymer_chains_with_consecutive_c_alpha_too_far_away(
        ...
        for index in range(len(chain_starts) - 1):
            ...
            if entity_poly_type.get(entity_id, "non-poly") == "polypeptide(L)":
                ...
                dist_square = np.sum((ca_atoms[:-1].coord - ca_atoms[1:].coord) ** 2, axis=-1)
    ...
    def too_many_chains_filter(
        ...
        is_resolved_centre_atom = (atom_array.centre_atom_mask == 1) & atom_array.is_resolved
        ...
        if core_indices is None:
            index_shuf = np.random.default_rng(seed=42).permutation(len(resolved_centre_atom))
        else:
        ...
        for idx in index_shuf:
            ...
            neighbors_indices = cell_list.get_atoms(centre_atom.coord, radius=interface_radius)
    ...
    def remove_clashing_chains(
        ...
        for atom_idx, clashes in enumerate(query_result):
            ...
            bonded_atoms, _bond_types = resolved_centre_atom_array.bonds.get_bonds(atom_idx)
        ...
        in_core_clashing_chain_ids = [1 if "." not in chain_id else 0 for chain_id in chain_ids]
        ...
        for _cycle in range(len(chain_ids)):
            ...
            has_clash_chain = np.any(
                [clash_ratio > clash_ratio_threshold for clash_ratio in clash_ratio_per_chain]
            )
    ...
    def remove_asymmetric_polymer_ligand_bonds(
        ...
        bonded_entity_to_num_copies = {
            entity_key: len(chain_ids) for entity_key, chain_ids in bonded_entity_to_chains.items()
        }
        ...
        for i, j in inter_chain_bonds:
            entity_i = atom_array.label_entity_id[i]
            entity_j = atom_array.label_entity_id[j]
            i_copies = bonded_entity_to_num_copies[(tuple(sorted([entity_i, entity_j])), entity_i)]
            j_copies = bonded_entity_to_num_copies[(tuple(sorted([entity_i, entity_j])), entity_j)]
            ...
            for n in neighbors:
                if not atom_array.is_resolved[n]:
                    ...
                    if atom_array.is_resolved[group_idx].any():
                        continue
                    if (atom_array.chain_id[group_idx] != atom_array.chain_id[centre_idx]).any():
                        continue
                    if (atom_array.res_id[group_idx] != atom_array.res_id[centre_idx]).any():
                        continue
    ...
    @staticmethod
    def get_rep_chains_for_too_many_chains(atom_array: AtomArray, radius: float = 5.0) -> list[str]:
        """
        ...
        cell_list = struc.CellList(atom_array, cell_size=radius, selection=atom_array.is_resolved)
    ...
    def _filter_altloc_by_local_largest(atom_array: AtomArray) -> np.ndarray:
        ...
        for group in res_groups_list:
            ...
            alt_and_avg_occ = [
                (altloc_char, np.mean(occ_list)) for altloc_char, occ_list in occ_dict.items()
            ]
            sorted_altloc_chars = [
            ...
            for start, end in group:
                ...
                selected_mask[start:end] = (label_alt_id == chosen_char) + (label_alt_id == ".")
        return selected_mask

    @staticmethod
    def filter_altloc(atom_array: AtomArray, altloc: str = "local_largest") -> AtomArray:
        """
    ...
    @staticmethod
    def remove_too_far_away_chains(atom_array: AtomArray, dist_thres: float = 30.0) -> AtomArray:
        """
        ...
        for chain_id in np.unique(atom_array.chain_id[other_resolved_mask]):
            ...
            coords = atom_array.coord[chain_mask]
            if np.any((coords - xyz_min > -dist_thres) | (coords - xyz_max < dist_thres)):
        ...
        selected_chain_ids = np.unique(atom_array.chain_id[other_resolved_mask][in_dist_mask])
