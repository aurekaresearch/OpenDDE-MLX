...
if not hasattr(pdbx_convert, "PDBX_COVALENT_TYPES"):
    pdbx_convert.PDBX_COVALENT_TYPES = list(pdbx_convert.PDBX_BOND_TYPE_ID_TO_TYPE.keys())
...
class MMCIFParser:
    ...
    def replace_auth_with_label(atom_array: AtomArray) -> AtomArray:
        ...
        for chain_id in chain_ids:
            ...
            if atom_array.label_seq_id[chain_mask][0] != ".":
                ...
                res_starts = get_residue_starts(atom_array[chain_mask], add_exclusive_stop=True)
                num = 1
    ...
    def get_structure(
        ...
        if biotite_version >= version.parse("1.2.0"):
            ...
            if model > model_count:
                raise ValueError(
                    f"The file has {model_count} models, the given model {model} does not exist"
                )

            model_atom_site = pdbx_convert_any._filter_model(atom_site, model_starts, model)
        ...
        if "struct_conn" in block:
            conn_bonds = _parse_inter_residue_bonds(model_atom_site, block["struct_conn"])
    ...
    def find_non_ccd_leaving_atoms(
        ...
        for idx in indices_in_atom_array:
            ...
            remove_atom_names.append(component.atom_name[ref_neighbor_idx][non_exist_mask].tolist())
    ...
    def build_ref_chain_with_atom_array(
        ...
        for entity_id, chain in entity_chains.items():
            ...
            entity_residues[entity_id] = {r.res_id[0]: r for r in struc.residue_iter(chain)}
        return entity_residues

    def make_new_residue(self, atom_array, res_start, res_stop, annots_to_indices) -> AtomArray:
        """
        ...
        for i, central_atom_name in enumerate(old_atom_names):
            ...
            if central_atom_name in ref_residue.central_to_leaving_groups:
                leaving_groups = ref_residue.central_to_leaving_groups[central_atom_name]
                # removed only when there are leaving atoms.
...
class AddAtomArrayAnnot(object):
    ...
    @staticmethod
    def add_token_annotations(atom_array: AtomArray, entity_poly_type: dict[str, str]) -> AtomArray:
        """
    ...
    def add_atom_mol_type_mask(atom_array: AtomArray) -> AtomArray:
        ...
        atom_array.set_annotation("chain_mol_type", np.array(chain_mol_type, dtype=object))
    ...
    def add_distogram_rep_atom_mask(atom_array: AtomArray) -> AtomArray:
        ...
        if np.sum(atom_array.distogram_rep_atom_mask) != np.sum(atom_array.centre_atom_mask):
    ...
    def find_equiv_mol_and_assign_ids(
        ...
        for mol_id in np.unique(atom_array.mol_id):
            ...
            _, chain_starts = np.unique(atom_array.chain_id[mol_mask], return_index=True)
                    ...
                    for ref_mol_id in checked_mol_id:
                        ...
                        if len(mol_atom_name) == len(ref_atom_name):
                            if np.all(ref_atom_name == mol_atom_name):
                                mol_id_to_entity_mol_ids[mol_id] = mol_id_to_entity_mol_ids[
                                    ref_mol_id
                                ]
                                break
                            else:
