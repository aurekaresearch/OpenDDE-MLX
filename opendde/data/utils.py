...
def within_dist(query_atoms: AtomArray, env_atoms: AtomArray, distance: float) -> np.ndarray:
...
def get_polymer_polymer_bond(atom_array: AtomArray, entity_poly_type: dict[str, str]) -> np.ndarray:
    """
...
def _repair_terminal_oxt_coordinates(atom_array: AtomArray) -> int:
    ...
    chain_ids = atom_array.label_asym_id if "label_asym_id" in annotations else atom_array.chain_id
    ...
    for chain_id in np.unique(chain_ids[atom_array.mol_type == "protein"]):
        ...
        atom_matches = {
            atom_name: np.flatnonzero(terminal_mask & (atom_array.atom_name == atom_name))
        ...
        if not (
            _MIN_TERMINAL_C_OXT_DISTANCE <= ref_c_oxt_distance <= _MAX_TERMINAL_C_OXT_DISTANCE
            and ref_o_oxt_distance >= _MIN_TERMINAL_O_OXT_DISTANCE
        ):
            continue

        rotation = pred_frame @ ref_frame.T
        atom_array.coord[atom_indices["OXT"]] = pred_coordinates[0] + rotation @ ref_c_oxt
        repaired += 1
...
def save_structure_cif(
    ...
    Args:
        ...
        pred_coordinate (np.ndarray): The predicted coordinates [N_atom, 3].
    ...
    pred_pose = np.array(pred_coordinate, copy=True)
...
def get_lig_lig_bonds(atom_array: AtomArray, lig_include_ions: bool = False) -> np.ndarray:
    """
...
def superimpose(
    ...
    assert np.any(fixed.label_asym_id == fixed_asym_id), f"{fixed_asym_id=} not in fixed!"
    assert np.any(mobile.label_asym_id == mobile_asym_id), f"{mobile_asym_id=} not in mobile!"
...
class CIFWriter:
    ...
    def get_entity_poly_and_entity_poly_seq_block(
        ...
        for entity_id, label_asym_ids_str in zip(
            ...
            first_asym_chain = atom_array[atom_array.label_asym_id == first_label_asym_id]
                ...
                for i in chem_comp_field:
                    if i == "mon_nstd_flag":
                        if ccd in STD_RESIDUES and ccd not in ["N", "DN", "UNK"]:
                            mon_nstd_flag = "y"
                        elif ccd_cif[ccd]["chem_comp"]["type"].as_item() == "non-polymer":
                            mon_nstd_flag = "."
                        else:
                            mon_nstd_flag = "n"
    ...
    def save_to_cif(
        ...
        cif = pdbx.CIFFile(
            {os.path.basename(output_path).replace(".cif", "") + "_predicted_by_opendde": block}
        )
...
def make_dummy_feature(
    ...
    if "msa" in dummy_feats:
        features_dict["msa"] = np.nonzero(features_dict["restype"])[1][None, :]
        ...
        features_dict["profile"] = features_dict["restype"].copy()
        ...
        for key in [
            ...
            features_dict[key] = np.zeros((), dtype=np.int32)

    if "template" in dummy_feats:
        # 31 is the gap token.
        features_dict["template_restype"] = np.full(
            feat_shape["template_restype"], 31, dtype=np.int64
        )
        ...
        features_dict["template_aatype"] = np.full(
            feat_shape["template_aatype"], 31, dtype=np.int64
        )
        features_dict["template_atom_mask"] = np.zeros(
            feat_shape["template_atom_mask"], dtype=np.int32
        )
        for key in [
            ...
            features_dict[key] = np.zeros(feat_shape[key], dtype=np.float32)
...
def data_type_transform(
    feat_or_label_dict: dict[str, np.ndarray],
) -> dict[str, np.ndarray]:
    """Cast every ``IntDataList`` feature to int64."""
...
def pdb_to_cif(input_fname: str, output_fname: str, entry_id: str | None = None):
    ...
    for c_start, c_stop in zip(chain_starts[:-1], chain_starts[1:]):
        ...
        hetero_diff = np.where(
            atom_array.hetero[c_start : (c_stop - 1)] != atom_array.hetero[(c_start + 1) : c_stop]
        )
    ...
    for c_start, c_stop in zip(new_chain_starts[:-1], new_chain_starts[1:]):
        ...
        if uncont_res_starts[0].shape[0] > 0:
            for res_start_atom_idx in uncont_res_starts[0]:
                ...
                if atom_array.hetero[new_chain_start] and atom_array.hetero[new_chain_start - 1]:
...
def pad_to(arr: np.ndarray, shape: tuple, **kwargs) -> np.ndarray:
    ...
    if arr.ndim != len(shape):
        raise ValueError(f"arr and shape have different number of axes. {arr.shape=}, {shape=}")
