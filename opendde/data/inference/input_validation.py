...
def validate_sample_name(value: Any, *, job_index: int | None = None) -> str:
    ...
    if value.casefold() == "err":
        raise ValueError(f"Inference job name{location} {value!r} is reserved for error reports.")
    if value in {".", ".."} or "/" in value or "\\" in value or "\x00" in value:
        raise ValueError(
            f"Inference job name{location} must be a single safe path component; got {value!r}."
        )
    return value
...
def validate_inference_jobs(value: Any) -> list[dict[str, Any]]:
    ...
    if not isinstance(value, list) or not value:
        raise ValueError(
            f"Input JSON must be a non-empty top-level list, got {type(value).__name__}."
        )
    ...
    for index, job in enumerate(value):
        if not isinstance(job, dict):
            raise ValueError(f"Inference job {index} must be an object, got {type(job).__name__}.")
        ...
        if model_seeds is not None:
            if not isinstance(model_seeds, list):
                raise ValueError(f"modelSeeds for job {name!r} must be a list of integers.")
