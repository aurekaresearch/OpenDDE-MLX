...
from opendde.data.constants import STD_RESIDUES
...
def atom_array_to_input_json(
    ...
    # add covalent bonds
    atom_array = AddAtomArrayAnnot.add_token_mol_type(atom_array, parser.entity_poly_type)
    ...
    if include_discont_poly_poly_bonds:
        polymer_polymer_bond = get_polymer_polymer_bond(atom_array, parser.entity_poly_type)
        ...
        for atoms in token_bonds[:, :2]:
            ...
            for i in range(2):
                ...
                bond_dict[f"entity{i + 1}"] = int(
                    label_entity_id_to_entity_id_in_json[atom_array.label_entity_id[atoms[i]]]
                )
...
if __name__ == "__main__":
    ...
    parser.add_argument("-c", "--cif_file", type=str, required=True, help="The cif file to parse")
    parser.add_argument(
        "-j",
        "--json_file",
        type=str,
