...
class SampleDictToFeatures:
    ...
    def add_bonds_between_entities(self, atom_array: AtomArray) -> AtomArray:
        ...
        for bond_info_dict in self.input_dict["covalent_bonds"]:
            ...
            for idx, i in enumerate(["left", "right"]):
                entity_id = int(
                    bond_info_dict.get(f"{i}_entity", bond_info_dict.get(f"entity{idx + 1}"))
                )
                copy_id = bond_info_dict.get(f"{i}_copy", bond_info_dict.get(f"copy{idx + 1}"))
                position = int(
                    bond_info_dict.get(f"{i}_position", bond_info_dict.get(f"position{idx + 1}"))
                )
                atom_name = bond_info_dict.get(f"{i}_atom", bond_info_dict.get(f"atom{idx + 1}"))
                ...
                if isinstance(atom_name, int):
                    ...
                    entity_dict = list(self.input_dict["sequences"][int(entity_id - 1)].values())[0]
    ...
    def get_feature_dict(self) -> tuple[dict[str, np.ndarray], AtomArray, TokenArray]:
        ...
        featurizer = Featurizer(token_array, atom_array, include_discont_poly_poly_bonds=True)
