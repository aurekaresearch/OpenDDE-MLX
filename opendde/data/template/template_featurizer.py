...
class TemplateFeatureAssemblyLine:
    ...
    def assemble(
        ...
        for asym_id, info in bioassembly.items():
            ...
            if (entity_id not in polymer_entity_features[skip_chain]) or (
                ...
                if skip_chain or not templates:
                    template_features = TemplateFeatures.empty_template_features(num_tokens)
                else:
                ...
                template_features = _reduce_template_features(template_features, self.max_templates)
                if entity_id in safe_entity_ids:
                    polymer_entity_features[skip_chain][entity_id] = template_features

            if entity_id in safe_entity_ids:
                feats: MutableFeatureDict = dict(polymer_entity_features[skip_chain][entity_id])
            else:
        ...
        for chain in np_chains_list:
            chain["template_aatype"] = pad_to(chain["template_aatype"], (self.max_templates, None))
        ...
        for feature_name, v in merged_example.items():
            merged_example[feature_name] = v[: self.max_templates, standard_token_idxs, ...]
...
class Templates:
    ...
    def as_opendde_dict(self) -> BatchDict:
        ...
        all_unit_vectors = np.empty((num_templates, num_res, num_res, 3), dtype=np.float32)
        ...
        for i in range(num_templates):
            ...
            uv, bb_mask_2d = TemplateFeatures.compute_template_unit_vector(aatype, pos, mask)
...
def _reduce_template_features(template_features: FeatureDict, max_templates: int) -> FeatureDict:
            ...
            for i in range(count):
                ...
                if "ion" in info:
                    chain_seq = "X" * np.count_nonzero(
                        (atom_array.asym_id_int == aid) & atom_array.centre_atom_mask.astype(bool)
                    )
