...
def encode_template_restype(chain_type: str, sequence: str) -> List[int]:
    ...
    if chain_type == PROTEIN_CHAIN:
        return [
            TEMPLATE_PROTEIN_SEQ_TO_ID.get(r, TEMPLATE_PROTEIN_SEQ_TO_ID["X"]) for r in sequence
        ]
    if chain_type == RNA_CHAIN:
        return [TEMPLATE_RNA_SEQ_TO_ID.get(r, TEMPLATE_RNA_SEQ_TO_ID["N"]) for r in sequence]
    if chain_type == DNA_CHAIN:
        return [TEMPLATE_DNA_SEQ_TO_ID.get(r, TEMPLATE_DNA_SEQ_TO_ID["N"]) for r in sequence]
...
class TemplateParser:
    ...
    @staticmethod
    def _mmcif_loop_to_list(prefix: str, parsed_info: MmCIFDict) -> List[Dict[str, str]]:
    ...
    def _get_header(parsed_info: MmCIFDict) -> PdbHeader:
        ...
        if release_date:
            ...
            logger.warning("Could not determine release_date for %s", parsed_info.get("_entry.id"))
    ...
    def _get_protein_chains(parsed_info: MmCIFDict) -> Dict[ChainId, Sequence[Monomer]]:
        ...
        entity_poly_seqs = TemplateParser._mmcif_loop_to_list("_entity_poly_seq.", parsed_info)
        ...
        chem_comps = TemplateParser._mmcif_loop_to_dict("_chem_comp.", "_chem_comp.id", parsed_info)
        struct_asyms = TemplateParser._mmcif_loop_to_list("_struct_asym.", parsed_info)

        entity_to_mmcif_chains = collections.defaultdict(list)
        for asym in struct_asyms:
            entity_to_mmcif_chains[asym["_struct_asym.entity_id"]].append(asym["_struct_asym.id"])

        valid_chains = {}
        for entity_id, seq_info in polymers.items():
            # Check if any component is peptide-like.
            is_protein = any(
                "peptide" in chem_comps.get(m.id, {}).get("_chem_comp.type", "") for m in seq_info
            )
            if is_protein:
    ...
    def parse(
        *,
        ...
        try:
            ...
            seq_start_num = {cid: min(m.num for m in seq) for cid, seq in valid_chains.items()}
            ...
            for atom in TemplateParser._get_atom_site_list(parsed_info):
                ...
                if atom.mmcif_chain_id in valid_chains:
                    ...
                    if atom.hetatm_atom == "HETATM":
                        hetflag = (
                            "W" if atom.residue_name in ("HOH", "WAT") else f"H_{atom.residue_name}"
                        )

                    ins_code = (
                        atom.insertion_code if TemplateParser._is_set(atom.insertion_code) else " "
                    )
                    pos = ResiduePosition(
                    ...
                    seq_to_structure_mappings[atom.author_chain_id][seq_idx] = ResidueAtPosition(
                        position=pos,
            ...
            for cid, seq_info in valid_chains.items():
                ...
                auth_cid = mmcif_to_author_chain_id[cid]
                seq = "".join(
                    [PDBData.protein_letters_3to1.get(monomer.id, "X") for monomer in seq_info]
                )
    ...
    def parse(hhr_string: str) -> List[TemplateHit]:
        ...
        if block_starts:
            ...
            for i in range(len(block_starts) - 1):
                hits.append(HHRParser._parse_hit(lines[block_starts[i] : block_starts[i + 1]]))
        return hits
...
class HmmsearchA3MParser:
    ...
    @staticmethod
    def parse(query_seq: str, a3m_str: str, skip_first: bool = True) -> List[TemplateHit]:
        """
