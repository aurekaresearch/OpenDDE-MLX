...
from opendde.data.tools.kalign import Kalign
...
from opendde.utils.file_io import compat_pickle_load
...
class TemplateFeatures:
    ...
    def empty_template_features(num_res: int, num_dense: int = 24) -> FeatureDict:
        ...
        template_features = {
            "template_aatype": np.array([STD_RESIDUES_WITH_GAP["-"]] * num_res, dtype=np.int32)[
                None, ...
            ],
            "template_atom_mask": np.zeros((num_res, num_dense), dtype=np.int32)[None, ...],
            "template_atom_positions": np.zeros((num_res, num_dense, 3), dtype=np.float32)[
                None, ...
            ],
    ...
    def pseudo_beta_fn(
        ...
        pseudobeta_index_polymer = np.take(RESTYPE_PSEUDOBETA_INDEX, aatype, axis=0).astype(
            np.int32
        )
    ...
    @staticmethod
    def dgram_from_positions(positions: np.ndarray, config: DistogramFeaturesConfig) -> np.ndarray:
        """Compute distogram from amino acid positions."""
        cache_key = (config.min_bin, config.max_bin, config.num_bins)
        if cache_key not in TemplateFeatures._dgram_cache:
            lower = np.linspace(config.min_bin, config.max_bin, config.num_bins, dtype=np.float32)
...
class TemplateHitProcessor:
    ...
    def _check_residue_distances(self, pos: np.ndarray, mask: np.ndarray, max_dist: float):
    ...
    def _extract_template_features(
        ...
        warning = None
        try:
            all_pos, all_mask = self._get_atom_positions(mmcif_obj, chain_id, 150.0, _zero_center)
        ...
        if np.sum(out_mask) < 5:
            raise TemplateAtomMaskAllZerosError(f"Empty atom mask for {pdb_id}_{chain_id}")
    ...
    def process(
        ...
        if self._template_cache_dir:
            cache_path = os.path.join(self._template_cache_dir, f"{pdb_id}_{chain_id}.pkl")
        ...
        try:
            (
                ...
                mapping,
                actual_chain_id,
            ) = self._align_query_to_hit_index_mapping(query_seq, chain_id, res.mmcif_object, 0.0)
            ...
            feats["template_sum_probs"] = [hit.sum_probs if hit.sum_probs is not None else 0.0]
...
class TemplateHitFeaturizer:
    ...
    def __init__(
        ...
        if max_template_date:
            if isinstance(max_template_date, str):
                self._max_template_date = datetime.strptime(max_template_date, "%Y-%m-%d")
            else:
    ...
    def get_templates(
        ...
        valid_hits.sort(key=lambda x: x.sum_probs if x.sum_probs is not None else 0.0, reverse=True)
