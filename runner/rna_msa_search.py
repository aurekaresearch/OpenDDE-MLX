...
def run_rna_msa_search(
    ...
    if nhmmer_binary_path is None:
        ...
        if not os.path.exists(nhmmer_binary_path):
            raise AssertionError(f"nhmmer binary path {nhmmer_binary_path} does not exist")
    ...
    if hmmalign_binary_path is None:
        ...
        if not os.path.exists(hmmalign_binary_path):
            raise AssertionError(f"hmmalign binary path {hmmalign_binary_path} does not exist")
    ...
    if hmmbuild_binary_path is None:
        ...
        if not os.path.exists(hmmbuild_binary_path):
            raise AssertionError(f"hmmbuild binary path {hmmbuild_binary_path} does not exist")
    ...
    if ntrna_database_path is None:
        ...
        ntrna_database_path = (
            _HOME_DIR / "search_database" / "nt_rna_2023_02_23_clust_seq_id_90_cov_80_rep_seq.fasta"
        ).as_posix()
    if not os.path.exists(ntrna_database_path):
        ...
        download_from_url(NT_SEARCH_DATABASE_URL, ntrna_database_path, check_weight=False)

    if rfam_database_path is None:
        _HOME_DIR = pathlib.Path(os.environ.get("OPENDDE_ROOT_DIR", default_root_dir()))
        rfam_database_path = (
            _HOME_DIR / "search_database" / "rfam_14_9_clust_seq_id_90_cov_80_rep_seq.fasta"
        ).as_posix()
    if not os.path.exists(rfam_database_path):
        ...
        download_from_url(RFAM_SEARCH_DATABASE_URL, rfam_database_path, check_weight=False)

    if rna_central_database_path is None:
        _HOME_DIR = pathlib.Path(os.environ.get("OPENDDE_ROOT_DIR", default_root_dir()))
        rna_central_database_path = (
            _HOME_DIR / "search_database" / "rnacentral_active_seq_id_90_cov_80_linclust.fasta"
        ).as_posix()
    ...
    logger.info(f"RNA MSA search done!, using {(rna_msa_end_time - rna_msa_start_time):.2f}s")
    ...
    for task_idx, infer_data in enumerate(json_data):
        ...
        for sequence_idx, sequence in enumerate(infer_data["sequences"]):
            if "rnaSequence" in sequence:
                ...
                if "unpairedMsaPath" in rna_chain and os.path.exists(rna_chain["unpairedMsaPath"]):
                    continue

                logger.info(
                    f"Running RNA MSA search for task {task_name}, rna_sequence: {rna_sequence}"
                )
                rna_output_dir = os.path.join(out_dir, task_name, "rna_msa", str(sequence_idx))
                run_rna_msa_search(
...
if __name__ == "__main__":
    example_seq = "GGCGCGUUAACAAAGCGGUUAUGUAGCGGAUUGCAAAUCCGUCUAGUCCGGUUCGACUCCGGAACGCGCCUCCA"
    run_rna_msa_search(
