...
def run_template_search(
    ...
    if msa_for_template_search_paths is None:
        ...
        msa_paths = list(dict.fromkeys(os.fspath(path) for path in msa_for_template_search_paths))
        if not msa_paths:
    ...
    if seqres_database_path is None:
        ...
        _SEQRES_DATABASE_PATH = _HOME_DIR / "search_database" / "pdb_seqres_2022_09_28.fasta"
    ...
    if not os.path.exists(seqres_database_path):
        ...
        download_from_url(TEMPLATE_SEARCH_DATABASE_URL, seqres_database_path, check_weight=False)
    ...
    logger.info(f"Template search done!, using {template_end_time - template_start_time}")
        ...
        for sequence in infer_data["sequences"]:
            if "proteinChain" in sequence:
                ...
                if basenames <= standard_names:
                    ...
                    source_identity = "\0".join(os.path.abspath(path) for path in msa_paths).encode(
                        "utf-8"
                    )
